...
router.get('/', async (req, res) => {
    ...
    switch (zipAPICall.status){
        ...
        default:
            ...
            const zipsString = nearbyZIPs.join();

            const clubs = await sequelize.query(`SELECT * FROM Club WHERE (select User.zipcode from Club inner join User on Club.owner_id = User.id) IN (${zipsString})`, { type: QueryTypes.SELECT });
            console.log(clubs);
            break;
    }
});
